...
const ACTIVITY_LABEL: Record<string, { label: string; icon: React.ComponentType<{ className?: string }>; className: string }> = {
  document_uploaded:        { label: "Documento criado",         icon: FileText,      className: "text-blue-500" },
  ...
  signature_requested:      { label: "Solicitação enviada",      icon: Send,          className: "text-primary" },
  ...
  document_ready:           { label: "Documento pronto",         icon: CheckCircle2,  className: "text-green-600" },
...
/* ---------- component ---------- */
interface Props {
  contrato: Contrato & { assinafy_assignment_id?: string | null; assinafy_signer_id?: string | null }
  open: boolean
  onClose: () => void
}
...
export function ContratoDetalheSheet({ contrato, open, onClose }: Props) {
  ...
  const isAssinado = contrato.status === "assinado" || docDetalhes?.status === "certificated"
    ...
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto">
        ...
        {contrato.assinafy_document_id && (
          <div className="mb-4 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {isAssinado && (
                <a
                  href={`/api/assinafy/download?document_id=${contrato.assinafy_document_id}&artifact=certificated`}
                  target="_blank"
              ...
              <a
                href={`/api/assinafy/download?document_id=${contrato.assinafy_document_id}&artifact=original`}
                target="_blank"
                rel="noopener noreferrer"
        ...
        {contrato.url_assinatura && contrato.status === "pendente" && (
          <div className="mb-4">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-1.5">Link de assinatura</p>
        ...
        {signers.length > 0 && (
          <div className="mb-4">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">
              Signatários {signers.length}/{signers.length}
            </p>
              ...
              {signers.map((sig) => (
                <div key={sig.id} className="flex items-center gap-3 p-3 bg-muted rounded-lg">
                  ...
                  {sig.status === "signed" || sig.signed_at ? (
                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-green-500/10 text-green-600 dark:text-green-400 shrink-0">
                      <CheckCircle2 className="w-3 h-3" /> Assinado
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 shrink-0">
                  ...
                  return (
                    <div key={a.id ?? i} className="flex items-start gap-3 relative">
                      ...
                      <div className="flex-1 min-w-0 -mt-0.5">
                        ...
                        {a.actor?.name && !desc && (
                          <p className="text-[11px] text-muted-foreground mt-0.5">{a.actor.name}</p>
                        )}
                        <p className="text-[10px] text-muted-foreground/70 mt-0.5">{formatDate(a.created_at)}</p>
                      </div>
                    </div>
                  )
                })}
