...
export async function GET(request: NextRequest) {
  ...
  // Parâmetros do paciente: codigoPaciente (não idPaciente)
  if (incoming.get("idPaciente")) sp.set("codigoPaciente", incoming.get("idPaciente")!)
  if (incoming.get("codigoPaciente")) sp.set("codigoPaciente", incoming.get("codigoPaciente")!)
  ...
  try {
    const url = `${CNN_BASE}/agenda/lista?${sp.toString()}`
    ...
    if (!res.ok) {
      ...
      console.error(`[cnn/agenda] ${res.status}:`, body)
    ...
    console.error("[cnn/agenda] erro:", e)
